...
def _build_communication_graph_optimized(df_window: pd.DataFrame) -> Dict[str, Any]:
    ...
    out_bytes = pd.to_numeric(df_window.get("bytes_sent", df_window.get("src_bytes", pd.Series(index=df_window.index))), errors="coerce").fillna(0.0).to_numpy(dtype=float)
    in_bytes = pd.to_numeric(df_window.get("bytes_received", df_window.get("dst_bytes", pd.Series(index=df_window.index))), errors="coerce").fillna(0.0).to_numpy(dtype=float)
    out_packets = pd.to_numeric(df_window.get("packets_sent", df_window.get("total_packets", pd.Series(index=df_window.index))), errors="coerce").fillna(0.0).to_numpy(dtype=float)
    in_packets = pd.to_numeric(df_window.get("packets_received", pd.Series(index=df_window.index)), errors="coerce").fillna(0.0).to_numpy(dtype=float)
